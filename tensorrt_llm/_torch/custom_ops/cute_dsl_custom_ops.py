...
if IS_CUTLASS_DSL_AVAILABLE:
    ...
    class CuteDSLBf16BlackwellBmmFp8OutRunner(TunableRunner):
        ...
        def forward(
            ...
            a_ptr = make_ptr(cutlass.BFloat16, a_tensor.data_ptr(),
                             cute.AddressSpace.gmem, assumed_align=16)
            b_ptr = make_ptr(cutlass.BFloat16, b_tensor.data_ptr(),
                             cute.AddressSpace.gmem, assumed_align=16)
            c_ptr = make_ptr(cutlass.Float8E4M3FN, c_tensor.data_ptr(),
                             cute.AddressSpace.gmem, assumed_align=16)
            stream = cuda.CUstream(torch.cuda.current_stream().cuda_stream)
