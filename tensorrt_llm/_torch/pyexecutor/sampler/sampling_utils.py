...
def sampling_batch_spec_dec_one_model(
    ...
    # token, i.e. the argmax of the original logits. This replaces the previous
    # unconditional logits.argmax(dim=-1): inductor fused that [tokens, vocab]
    # reduction with the temperature divide into a persistent-reduction kernel
    # (one CTA per row) costing ~1 ms/step at small batch for a 154k vocab --
    # paid even by all-non-greedy batches whose argmax result was then discarded
    # by the final where(). All ops remain branch-free (no data-dependent
    # control flow), so this stays CUDA-graph safe.
