...
protected:
        ...
        std::optional<std::vector<bool>> const& indexerKCacheLayerMask = std::nullopt)
    {
        ...
        // Per-PP indexer K cache layer counts for a compact indexer pool (per-layer indexer
        ...
        bool isIndexerKCache = false)
    {
        ...
        {
            // The (possibly compact) indexer pool lives in "indexer layer space": one row per
...
// Compact indexer K cache pool (per-layer indexer mask, GLM 5.2-style cross-layer indexer
// sharing): only full-indexer layers own a pool row, so the indexer pass runs in "indexer
// layer space". Exercises PP resharding of compact indexer blocks end to end.
class AsymmetricalCacheTestWithCompactIndexer : public AsymmetricalCacheTest
{
};

TEST_P(AsymmetricalCacheTestWithCompactIndexer, TestCase)
{
    AsymmetricTestParam param = GetParam();
    int contextTp = std::get<0>(param);
    int contextPp = std::get<1>(param);
...
// ctx/gen PP resharding exchanges compact indexer blocks with asymmetric per-PP counts.
INSTANTIATE_TEST_CASE_P(AsymmetricCaseTest0ForMLAWithCompactIndexerKCache, AsymmetricalCacheTestWithCompactIndexer,
    testing::Combine(testing::Values(1), testing::Values(1, 2), testing::Values(1), testing::Values(1),
        ...
        testing::Values(16), testing::Values(nvinfer1::DataType::kFLOAT), testing::Values(1), testing::Values(true),
        testing::Values(false), testing::Values(false), testing::Values(false), testing::Values(true),
