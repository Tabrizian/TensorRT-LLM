...
public:
    ...
    //! indexer pool (compact layout); rows follow the order of `managedLayers` restricted to
    ...
    //! \brief Row of `layerIdx` within the compact indexer K cache pool, or -1 when the layer
...
private:
    ...
    // When set, only masked-in layers get a row in the (compact) indexer pool —
    ...
    // layerIdx -> row within the compact indexer K cache pool (only masked-in layers).
