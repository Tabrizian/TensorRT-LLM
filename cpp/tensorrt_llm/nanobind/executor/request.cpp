...
void initRequestBindings(nb::module_& m)
{
    ...
    auto requestGetstate = [](tle::Request const& self)
    {
        ...
        auto inputTokenIdsBytes = nb::bytes(reinterpret_cast<char const*>(inputTokenIds.data()),
            inputTokenIds.size() * sizeof(VecTokens::value_type));
    ...
    auto requestSetstate = [](tle::Request& self, nb::tuple const& state)
    {
        ...
        new (&self) tle::Request(std::move(inputTokenIds), nb::cast<SizeType32>(state[1]),
