...
TargetRanksInfo targetIRanksForIndexerKCache(
    kv_cache::CacheState const& peerCacheState, kv_cache::CacheState const& selfCacheState, int selfRank)
{
    // The indexer K cache pass shares the attention pass's connection set, so keep the
    // attention-domain rank topology and only recompute the per-peer layer counts in
    // "indexer layer space". Both sides derive the same global full-indexer-layer schedule
    // from the model config, so the per-PP indexer counts are prefix-consistent between the
    // two sides: the count exchanged with a peer equals the number of full-indexer layers
    // inside the attention-layer overlap, i.e. the interval intersection of the indexer
    // prefix sums (monotone images of the attention prefix sums).
    ...
    bool isIndexerKCache = false)
{
    ...
    // For the indexer K cache pass only full-indexer layers own a pool row (compact layout),
