...
void WindowBlockManager::createIndexerKCachePools(std::vector<SizeType32> const& managedLayers)
{
    ...
    for (SizeType32 i = 0; i < numPools; ++i)
    {
        ...
        // compact row. Rows follow managedLayers order restricted to this pool, matching the
        ...
        {
            TLLM_LOG_INFO("[%s] Compact indexer K cache pool: %d of %d layers own an indexer K cache.",
                mLogPrefix.c_str(), numIndexerLayers, kvPool.numLayers);
        }
